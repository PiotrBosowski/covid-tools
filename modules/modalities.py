...
def split_by_modality(args):
    """
    split_dataset_by_modality() wrapper.
    """
...
def split_dataset_by_modality(dataset_path, image_extension):
    """
    AI-powered splitter by modality. Current version supports only
    splitting medical chest images, but can be easly trained to
    distinguish any desired modalities.
    """
    ...
    images = [os.path.join(dataset_path, image)
    ...
    with torch.no_grad():
        ...
        for index, image in enumerate(images):
            try:
                ...
                shutil.copy(image, os.path.join(dataset_path,
                                                labels[prediction.item()],
                                                os.path.basename(image)))
            except Exception as ex:
                print(f"Error processing image {os.path.basename(image)},"
                      f"{ex}")


if __name__ == '__main__':
    split_dataset_by_modality(
        '/home/peter/media/data/covid-19/cr/03_BIMCV-COVID-19-nega/'
        'converted/covid-or-healthy', '.png')